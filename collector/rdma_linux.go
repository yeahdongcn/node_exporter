...
func makeRdmaCollector(logger log.Logger) (*rdmaCollector, error) {
	if *rdmaDeviceInclude != "" {
		level.Info(logger).Log("msg", "Parsed flag --collector.rdma.device-include", "flag", *rdmaDeviceInclude)
	}
	if *rdmaDeviceExclude != "" {
		level.Info(logger).Log("msg", "Parsed flag --collector.rdma.device-exclude", "flag", *rdmaDeviceExclude)
	}
...
func NewRdmaCollector(logger log.Logger) (Collector, error) {
...
func (c *rdmaCollector) Update(ch chan<- prometheus.Metric) error {
	...
	for _, device := range rdmaDevices {
		...
		if err != nil {
			level.Error(c.logger).Log("msg", "rdma stats error", "err", err, "device", device)
			continue
		}
		...
		updateFunc := func(name string, value float64, labelValues ...string) {
			...
			entry := c.entry(name)
			if entry == nil {
				level.Warn(c.logger).Log("msg", "rdma metric not found", "name", name)
				return
			}
